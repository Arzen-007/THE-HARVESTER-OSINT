...
import { Zap, Loader2, Bug, AlertCircle } from "lucide-react";
import { toast } from "sonner";

export default function NucleiScan() {
  const [target, setTarget] = useState("");
  const startScan = trpc.osint.startScan.useMutation({
    onSuccess: (data) => {
      toast.success(`Nuclei scan started! ID: ${data.scanId}`);
    },
    onError: (error) => {
  ...
  const handleScan = (e: React.FormEvent) => {
    e.preventDefault();
    if (!target) return;
    startScan.mutate({ tool: "nuclei", target });
  };
  ...
  return (
    <div className="border-glow rounded-lg p-6 bg-black/40 border border-green-500/20">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-xl font-cyber tracking-wider text-green-400">NUCLEI VULN SCAN</h2>
      </div>
      ...
      <form onSubmit={handleScan} className="space-y-4">
        <div>
          ...
          <div className="relative">
            <Zap className="absolute left-3 top-2.5 w-4 h-4 text-green-800" />
            <input
              type="text"
              value={target}
      ...
      <div className="mt-6 p-4 bg-green-950/10 border border-green-900/20 rounded text-[10px] font-mono-code text-green-700">
        <div className="flex items-start gap-2">
          ...
          <p>
            Nuclei uses community-powered templates to find vulnerabilities. It covers 
            misconfigurations, CVEs, and exposed panels.
          </p>
        </div>
      </div>
    </div>
